...
def menu():
    ...
    return render_template('menu.html',
                           plats=lesPlats,
                           TypeDePlats=lesTypeDePlats,
                           formules=lesFormules)
...
def contact():
    if request.method == "POST":
        ...
        msg = Message(
            ...
            sender=app.config[
                "MAIL_DEFAULT_SENDER"],  # fonctionne car on s'envoie le mail a nous meme pas besion de verif l'adresse de l'auteur
...
def get_or_create_panier(idUser):
    ...
    """
    panier = Reservation.query.filter_by(idUser=idUser,
                                         statut="EN ATTENTE").first()
    if not panier:
        panier = Reservation(idUser=idUser,
                             dateR=datetime.now(),
                             nb_couverts=1,
                             sur_place=False,
                             statut="EN ATTENTE")
...
def update_checkbox():
    ...
    resa = Reservation.query.filter_by(idR=idR,
...
@login_required
def preparer_panier(idR, action):
